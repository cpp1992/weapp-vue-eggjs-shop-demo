      ...
      <el-table-column label="操作" fixed="right" width="100" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            size="mini"
            @click="deleteFreightPlan(scope.row.uuid)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          if (dialogType === 'edit') {
            await this.$api.freightPlan.saveModify({
              freightPlan: { ...selectedData, ...dialogForm },
            });
            this.$message({ message: '修改运费方案成功', type: 'success' });
            this.hideDialog();
            this.query();
          } else {
            await this.$api.freightPlan.saveNew({
              freightPlan: this.dialogForm,
            });
