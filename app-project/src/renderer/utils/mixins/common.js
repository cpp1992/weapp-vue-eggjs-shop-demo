...
export const pageMixin = {
  ...
  activated() {
    const refreshDataCallbackMap = this[
    ...
    if (!_.isEmpty(refreshDataCallbackMap)) {
      Object.entries(refreshDataCallbackMap).forEach((item) => {
        ...
        if (
          ...
          this.$store.commit('setRefreshDataMap', {
            key,
            value: keepAliveNames,
          });
        }
      });
    }
  ...
  computed: {
    ...mapState({
      ...
      mx_isRefreshPage(state) {
        ...
        return (
          currentRouteName === this.$options.name
          && state.refreshPageMap[currentRouteName]
        );
      },
    }),
  },
...
export const tableMixin = {
  ...
  methods: {
    ...
    mx_getTableParams(target) {
      ...
      this.$set(
        targetTable.params.pagination,
        'page',
        targetTable.pagination.page,
      );
      this.$set(
        targetTable.params.pagination,
        'pageSize',
        targetTable.pagination.pageSize,
      );
